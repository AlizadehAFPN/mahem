...
export function SingleJobScreen() {
    ...
    const jobObj = useMemo(() => {
        if (job) {
            const { manager, register_code, phone, mobile, fax, address, telegram, instagram, email, description, job_category_id } = job
            return ([
        ...
        <Screen withoutScroll>
            ...
            <Screen unsafe>
                ...
                <View style={styles.bannerContaier}>
                    <Image style={{ width: '100%', height: '100%' }} source={{ uri: job?.banner?.path }} />
                </View>
                <View style={styles.grayCard}>
                    <View style={styles.circle}>
                        <Image style={{ height: '100%', width: '100%', }} source={{ uri: job?.logo?.path }} />
                    </View>
                </View>
                <Divider height={8} />
                {jobObj.map(item =>
                    <Row style={{ paddingHorizontal: 8 }}>
                        ...
                        <View style={{ ...styles.detailItem, flex: 1 }}>
                            <Text style={{ ...styles.itemText, textAlign: 'right' }}>{item.value}</Text>
                        </View>
                    </Row>
                )}
                <ProductLocation
