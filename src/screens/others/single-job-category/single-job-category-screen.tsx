...
import { Button, Divider, Row, Screen, TableRow, Text, TextField } from '../../../components'
...
export function SingleJobCategoryScreen() {
    ...
    const {
        ...
        queryFn: ({ pageParam = 1 }) => getAllJobs({page: pageParam, category_id: params?.category?.id}),
    ...
    const result = useMemo(()=>{
        if(searchText){
            return jobs.filter(item=> JSON.stringify(item).includes(searchText.toLowerCase()))
        }
        return jobs
    }, [jobs,searchText ])
    ...
    return (
        <Screen withoutScroll style={{ flex: 1 }}>
            ...
            <View style={styles.iconContainer}>
                <Image style={{ width: '80%', height: '80%' }} source={{uri: category?.logo}} />
            </View>
            <View style={styles.titleBar}>
                <Text style={{ textAlign: "center" }} size={17}>{category?.title}</Text>
                <Divider />
            ...
            <FlatList
                ...
                renderItem={({ item, index }) => {
                    return <TableRow onPress={() => handlePressItem(item)} item={[index, item?.title, item?.manager, item?.phone]} />
                }}
                ListHeaderComponent={
                    <TableRow
                        header
                        item={['ردیف', 'نام واحد', 'مدیریت', 'تلفن']}
                    />}
