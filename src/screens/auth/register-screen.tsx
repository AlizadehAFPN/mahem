...
import { Button, Divider,
     FilePickerModal, 
     Screen, Text, TextField } from '../../components'
...
export function RegisterScreen() {
        ...
        if (state.mobile) {
            ...
            timeoutRef.current = setTimeout(() => {
                if (!mobileValidation(state.mobile)) {
                    setState(s => ({ ...s, mobileError: "شماره موبایل خود را به درستی وارد کنید!" }))
                }
            }, 500);
        }
    }, [state.mobile])
    ...
    const enableButton = useCallback(handleValidation, [state.mobile, state.username])
    return (
        <Screen  statusbarBackgroundColor={colors.main}>
            <View style={sytles.topColor}>
                <Text preset='default' size={20} color="white">برای ثبت نام کافیست شماره همراه خود را وارد نمایید تا کد فعال سازی برایتان ارسال شود.</Text>
            </View>
            <View style={sytles.grayCard}>
                <TouchableOpacity onPress={() => setState(s => ({ ...s, pickerModal: true }))} style={sytles.cammeraButton}>
                    {state?.profileImage?.uri ? <Image
            ...
            <View style={sytles.formContainer}>
                ...
                />
                <Divider height={80} />
                <Button loading={isLoading} disabled={!enableButton()|| isLoading} onPress={onPressRegister} style={{...sytles.button, backgroundColor: enableButton()? colors.main: colors.pallete.gray1}}>
                    <Text color={enableButton()? "white": "black"} size={20}>ثبت نام</Text>
                </Button>
            </View>
            <FilePickerModal
