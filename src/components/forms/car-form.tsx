...
export function CarForm({ subCategory, send, onSend }) {
    ...
    useEffect(() => {
        if (send?.includes("send")) {
            ...
            if (isValid) {
                const { title, description, price, adsType, contact_info, city, brand, operation_amount, product_year, payType,  } = state
                const ad_type = optionsTypes.adsType.findIndex(item => item.title === adsType)
                const tempB = optionsTypes.adsType.findIndex(item => item.title === brand)
                const is_cash = payType == "اقساطی"
                onSend({ title, description, price, contact_info, city_id: city?.id, operation_amount, product_year, is_cash, ad_type, brand: tempB })
            } else {
                onSend(false)
            }
        }
        ...
        <>
            <View>
                ...
                {subCategory?.title?.includes('سواری') && <>
                    <Divider />
                    <Button onPress={() => setState(s => ({ ...s, optionModal: true, optionType: 'brand' }))}>
                        <UnderlineTextField
                            onPressIn={() => setState(s => ({ ...s, optionModal: true, optionType: 'brand' }))}
                            value={state.brand}
                    ...
                    <Divider />
                    <Button onPress={() => setState(s => ({ ...s, optionModal: true, optionType: 'chassi' }))}>
                        <UnderlineTextField
                            onPressIn={() => setState(s => ({ ...s, optionModal: true, optionType: 'chassi' }))}
                            value={state.chassi}
                            keyboardType="number-pad"
                ...
                <Divider />
                <Button onPress={() => setState(s => ({ ...s, optionModal: true, optionType: 'payType' }))}>
                    <UnderlineTextField
                        onPressIn={() => setState(s => ({ ...s, optionModal: true, optionType: 'payType' }))}
                        value={state.payType}
                        keyboardType="number-pad"
                ...
                <Divider />
                <Button onPress={() => setState(s => ({ ...s, optionModal: true, optionType: 'carAdsCreator' }))}>
                    <UnderlineTextField
                        editable={false}
