...
export function EmployeeForm({ send, onSend }) {
    ...
    useEffect(() => {
        if (send?.includes("send")) {
            ...
            if (isValid) {
                const { title, description, education, contractType, features, contact_info, city } = state
                const degree = optionsTypes.education.findIndex(item => item.title === education)
                const contract_type = optionsTypes.contractType.findIndex(item => item.title === contractType)
                onSend({ title, description, features, contact_info, city_id: city?.id, degree, contract_type })
            } else {
                onSend(false)
            }
        }
    ...
    return (
        <>
            <View>
                ...
                <Divider />
                <Button onPress={() => setState(s => ({ ...s, optionModal: true, optionType: 'education' }))}>
                    <UnderlineTextField
                        onPressIn={() => setState(s => ({ ...s, optionModal: true, optionType: 'education' }))}
                ...
                <Divider />
                <Button onPress={() => setState(s => ({ ...s, optionModal: true, optionType: 'contractType' }))}>
                    <UnderlineTextField
                        onPressIn={() => setState(s => ({ ...s, optionModal: true, optionType: 'contractType' }))}
